...
async def stream_response(
    websocket: WebSocket, payload: dict, query: str, session_id: str, user_id: str
):
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            await websocket.send_text(
                json.dumps(
                    {"type": "query_response", "content": error_msg, "done": True}
                )
            )
        ...
        while True:
            ...
            with tracer.start_as_current_span(f"{msg_type}", context=ctx) as span:
                ...
                if msg_type == "logout":
                    ...
                    print("Before mongo:", type(span), "get_span_context" in dir(span))
                    ctx_for_mongo = set_span_in_context(span)
                    print("After get:", type(ctx_for_mongo))  # Should be SpanContext
                    mongo_span = tracer.start_span("mongo:login", context=ctx_for_mongo)
                    print("After start:", type(mongo_span))
                    ...
                    await stream_response(websocket, payload, query, session_id, user_id)
