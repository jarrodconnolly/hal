...
def extract_markdown_paragraphs_parallel(file_path, stats, max_workers=20):
    ...
    """
    print(f"Processing {file_path} (parallel sections)...")
    start_time = time.time()
    ...
    print(f"Loaded/stripped Markdown in {time.time() - start_time:.2f} seconds")
...
def score_chunk(chunk_text):
    ...
    try:
        ...
        logging.warning(f"spaCy failed on chunk: {str(e)} - returning 0.0 score")
        return 0.0
    score = 0.0
...
def process_files(file_paths, stats):
    ...
    print(f"Filtered {len(all_chunks) - len(filtered_chunks)} chunks scoring <0.3")
...
def update_vector_store(data_dir, output_dir):
    ...
    if os.path.exists(state_file):
        ...
        if client.collection_exists(DOCS_COLLECTION):
            print("No state.json found—resetting collection.")
    ...
    if deleted_files:
        for fp in deleted_files:
            ...
            if chunk_ids:
                print(f"Deleting {len(chunk_ids)} chunks for {fp}")
                client.delete(
                    collection_name=DOCS_COLLECTION,
    ...
    for idx, (embedding, chunk) in enumerate(zip(embeddings, new_chunks)):
        ...
        if failed_batches:
            ...
            logging.info("All batches upserted successfully")
