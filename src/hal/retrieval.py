...
def create_collections() -> None:
    ...
    if not client.collection_exists(FACTS_COLLECTION):
        logging.info(f"Creating collection {FACTS_COLLECTION}")
    ...
    if not client.collection_exists(HISTORY_COLLECTION):
        logging.info(f"Creating collection {HISTORY_COLLECTION}")
        client.create_collection(
...
def add_to_history(query: str, answer: str, session_id: str) -> None:
    content = f"Q: {query}\nA: {answer}"
...
def store_user_facts(facts: list[str], session_id: str, source_query: str) -> None:
    ...
    for fact in facts:
        embedding = embeddings.encode(fact)
        point_id = f"{session_id}_{int(time.time() * 1000)}"  # Unique per user and millisecond
        try:
            client.upsert(
                collection_name=FACTS_COLLECTION,
                points=[
            ...
            logging.info(f"Stored fact: {fact} for session_id: {session_id}")
        except Exception as e:
            logging.error(f"Error storing fact '{fact}': {e}")
            raise
...
def get_user_facts(user_id: str, limit: int = 5) -> list[str]:
    ...
    try:
        ...
        logging.info(f"Retrieved {len(facts)} facts for user_id: {user_id}")
        return facts
    except Exception as e:
        logging.error(f"Error retrieving facts for user_id {user_id}: {e}")
        return []
