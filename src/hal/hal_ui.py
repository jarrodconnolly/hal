...
class HALApp(App):
    ...
    async def login(self, username: str) -> dict:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if response.status_code == 200:
                ...
                return {"session_id": str(uuid.uuid4()), "chunk_count": 0, "error": "Login failed - using temp session"}
    ...
    def on_unmount(self) -> None:
        ...
        logging.info(f"Session ended for {self.session_id} - Total tokens: {self.token_count}")
    ...
    def on_key(self, event: Key) -> None:
        ...
        if event.key == "enter":
            ...
            char = ''.join(c for c in event.character if ord(c) >= 32)  # Strip control chars
    ...
    def on_paste(self, event: Paste) -> None:
        ...
        console = self.query_one("#console")
        scroll = self.query_one(VerticalScroll)
        pasted_text = ''.join(c for c in event.text.strip() if ord(c) >= 32)  # Strip control chars
        if pasted_text:
