...
function connectWebSocket() {
  ...
  ws.onmessage = (event) => {
    ...
    if (msgType === 'login_response' || msgType === 'logout_response' || msgType === 'query_response') {
      const spanId = message.traceparent.split('-')[2];
...
function logout() {
  ...
  //const { traceparent } = startSpan('logout');
  ws.send(JSON.stringify({ type: 'logout', session_id: sessionId }));
}
...
terminal.addEventListener('keydown', (e) => {
  if (e.key === 'Enter') {
    ...
    if (input.startsWith('/')) {
      ...
      //const { span, traceparent } = startSpan('query');
      //span.setAttribute('sessionId', sessionId);
      //span.setAttribute('userId', userId);
      ws.send(JSON.stringify({ type: 'query', session_id: sessionId, query: input }));
    }
